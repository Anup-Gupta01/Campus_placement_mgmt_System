...
const statusStyle: Record<string, string> = {
  ...
  Shortlisted:  "text-[#1F2937] bg-slate-100   border-slate-300",
  "OA Pending": "text-amber-700  bg-amber-50    border-amber-200",
  Interview:    "text-emerald-700 bg-emerald-50 border-emerald-200",
  Selected:     "text-emerald-700 bg-emerald-50 border-emerald-200",
  Rejected:     "text-red-700    bg-red-50      border-red-200",
};

function SkillTag({ skill }: { skill: string }) {
  return <span className="px-2 py-0.5 text-[11px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-lg">{skill.trim()}</span>;
}
...
export default function ManageApplications() {
  ...
  const statCards = [
    ...
    { label: "Shortlisted",      value: stats.shortlisted,     icon: CheckCircle2,color: "text-[#1F2937]",  bg: "bg-slate-100",  border: "border-slate-300"   },
    { label: "In Interview",     value: stats.inInterview,     icon: Clock,       color: "text-emerald-700", bg: "bg-emerald-50", border: "border-emerald-200" },
        ...
        <div className="flex flex-wrap items-center gap-3 mb-6">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search applicants..."
              className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-[14px] font-medium focus:outline-none focus:ring-2 focus:ring-emerald-600/20 focus:border-emerald-600 transition-all shadow-sm" />
          </div>
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
            className="px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-[14px] font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-600/20 focus:border-emerald-600 shadow-sm">
                        ...
                        <td className="px-5 py-4">
                          <div className="relative">
                            <select
                              ...
                              className={`text-[12px] font-bold px-3 py-1.5 rounded-lg border appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-emerald-600/20 transition-all pr-7 ${statusStyle[app.status] || "text-slate-600 bg-slate-100 border-slate-200"} ${updating === app._id ? "opacity-60" : ""}`}
                            >
                              {STATUS_OPTIONS.map(s => <option key={s} value={s}>{s}</option>)}
                            </select>
                          </div>
