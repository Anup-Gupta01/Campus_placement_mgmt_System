...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, string> = {
    ...
    Shortlisted:"text-[#1F2937] bg-emerald-50 border border-emerald-200/60",
    "OA Pending":"text-yellow-700 bg-yellow-50 border border-yellow-200/60",
    Interview:  "text-emerald-700 bg-emerald-50 border border-emerald-200/60",
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-black w-full font-sans pb-20">
      {loading && (
        <div className="fixed inset-0 bg-white/60 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="flex flex-col items-center gap-3">
            <div className="w-10 h-10 border-4 border-emerald-200 border-t-[#059669] rounded-full animate-spin" />
      ...
      <div className="max-w-[1400px] mx-auto px-6 sm:px-12 py-10 w-full">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-5 py-2.5 bg-white border border-slate-200 text-slate-700 text-sm font-bold rounded-xl shadow-sm hover:bg-slate-50 flex items-center gap-2 transition-all"
            >
              <Download className="w-4 h-4 mr-2" /> Export Report
            </button>
            <Link
              href="/admin/dashboard/postopportunity"
              className="px-5 py-2.5 bg-[#1F2937] hover:bg-[#111827] text-white text-sm font-bold rounded-xl shadow-sm flex items-center transition-all active:scale-[0.98]"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {[
            { label: "Total Students",       value: stats.totalStudents?.toLocaleString()  || "—", sub: "Registered students",     icon: Users,     bg: "bg-emerald-50",    iconClr: "text-emerald-600" },
            { label: "Active Opportunities", value: stats.activeOpportunities              || "—", sub: "Open job postings",        icon: Briefcase, bg: "bg-emerald-50",  iconClr: "text-emerald-600" },
            ...
            <div className="h-[280px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={trend} margin={{ top: 5, right: 20, bottom: 5, left: -20 }}>
                  ...
                  <Line type="monotone" dataKey="count" stroke="#059669" strokeWidth={3} dot={{ r: 5, fill: "#059669", strokeWidth: 2, stroke: "#fff" }} activeDot={{ r: 7, strokeWidth: 0, fill: "#047857" }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
            ...
            <div className="h-[280px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={branch} margin={{ top: 5, right: 20, bottom: 5, left: -20 }}>
                  ...
                  <Bar dataKey="count" fill="#D1FAE5" radius={[6, 6, 0, 0]} activeBar={{ fill: "#059669" }} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {data?.activeOpportunities?.length === 0 && (
                  <tr><td colSpan={7} className="px-7 py-10 text-center text-slate-400 font-medium">No active opportunities yet. <Link href="/admin/dashboard/postopportunity" className="text-emerald-600 font-bold">Post one!</Link></td></tr>
                )}
                {(data?.activeOpportunities || []).map((job: any) => (
                  <tr key={job._id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-7 py-5">
                      ...
                      <p className="text-[13px] font-medium text-emerald-600 mt-0.5">{job.role}</p>
                    </td>
                    ...
                    <td className="px-7 py-5 text-center">
                      <span className="text-[13px] font-bold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-lg">{job.shortlisted}</span>
                    </td>
                    <td className="px-7 py-5 text-center">
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-10 pr-4 py-2.5 bg-[#F8FAFC] border border-slate-200 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-emerald-600/20 focus:border-emerald-600 transition-all w-full sm:w-[280px]"
                />
              </div>
            </div>
          </div>
